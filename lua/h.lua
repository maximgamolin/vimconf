-- cheatsheet.lua
function Hlp()
  local buf = vim.api.nvim_create_buf(false, true)
  local cheatsheet = {
    '==============================',
    ' Переход к концу файла    : <C-End>  (G)',
    ' Переход к началу файла   : <C-Home> (gg)',
    ' Переход к концу слова    : <C-e>    (e)',
    ' Переход к началу слова   : <C-b>    (b)',
    ' Переход к концу строки   : <C-$>    ($)',
    ' Переход к началу строки  : <C-0>    (0)',
    ' Открыть терминал : <C-d>    (diw)',
    ' Скопировать строку yy, несколько строк 3yy',
    ' Вставить строку p',
    ' :FloatermNext - Следущий терминал',
    ' :FloatermKill - Закруть терминал',
    ' Ctrl-w h, Ctrl-w j, Ctrl-w k, Ctrl-w l для перехода между окнами (влево, вниз, вверх, враво соответственно)',
    ' \\ff поиск файлов; \\fb буферы; \\fh help tags',
    ' ============================== ',
    ' Поиск по содержимому файлов (Telescope live_grep)',
    ' <C-S-f> / <M-F>  открыть поиск (запрашивает расширение файла)',
    '   Примеры расширений: *.py  *.lua  *.js  (пусто = искать во всех)',
    '   Скрытые папки (.idea, .venv, .git и т.д.) в поиск не включаются',
    ' <Space> в дереве файлов → контекстное меню:',
    '   на файле: открыть, копировать путь, git add, частично добавить в гит, создать файл/папку',
    '   на папке: создать файл/папку внутри (путь вида foo/bar/buzz.py создаёт вложенные папки)',
    ' F5 открыть историю изменения файла',
    ' ============================== ',
    ' Сворачивание кода (folding)',
    ' za  свернуть/развернуть блок под курсором',
    ' zR  развернуть всё в файле',
    ' zM  свернуть всё в файле',
    ' F8 Открыть боковой бар с классами',
    ' \\nm карта кода справа (как в Sublime); \\nf фокус на карту',
    ' ]c / [c следующее/предыдущее git-изменение; \\hp diff куска; \\hr откатить кусок; \\hs stage куска',
    '==============================',
    ' Сочетания клавиш для автодополнения',
    ' Прокрутка документации вверх       : <C-d>',
    ' Прокрутка документации вниз        : <C-f>',
    ' Открыть автодополнение             : <C-Space>',
    ' Закрыть автодополнение             : <C-e>',
    ' Подтвердить выбор автодополнения   : <CR>',
    ' ==============================',
    ' v перейти в визуальный режим',
    ' "+y копировать в буфер обмена',
    ' "+p вставить из буфера обмена',
    ' \\cc закоментировать код',
    ' \\cu раскоментировать код',
    ' u отменить действие; Ctrl + R вернуть отмененное действие',
    ' ==============================',
    ' :noh отменить выделение после поиска по файлу',
    ' :42 - перейти к 42 строке',
    ' :%s/старый_текст/новый_текст/g - поиск и замена; в конце c - подтверждение на каждую замену, i - регистронезависимая замена',
    ' * выделяет слово целиком и ищет его по файлу',
    " n после поиска слова через слеш '/someword' и enter идет к следующему файлу",
    ' ============================== ',
    ' grr переименование всех использований переменных',
    ' gnd перейти к объявлению',
    ' <C-Enter>  на использовании → перейти к объявлению; на объявлении → показать все использования',
    ' * к следующему использованию',
    '- "_daw - удалить слово целиком, включая пробелы, не помещая в буфер вырезки.',
    '- "_diw - удалить слово, не захватывая пробелы, не помещая в буфер вырезки.',
    ' ============================== ',
    ' Локальное ревью (:ReviewOpen — ветка от merge-base с main; :ReviewPick — выбрать ветки/коммиты; :ReviewOpen A B — любые ревизии)',
    ' Клик по «ветка@hash ▾» в шапке diff — сменить базу (слева) или голову (справа): список фильтруется набором текста',
    ' Кнопки «‹ пред / след ›» в шапке — навигация по изменениям (то же, что ]c / [c в diff-окне)',
    ' \\rc замечание к строке/выделению; \\re править; \\rd удалить; \\rl список',
    ' ]r / [r следующее/предыдущее замечание; t переключить side-by-side/inline; q закрыть',
    ' :ReviewPublish закоммитить .review/; :ReviewDone закрыть замечания; /fix-review — Claude исправляет',
  }

  vim.api.nvim_buf_set_lines(buf, 0, -1, false, cheatsheet)
  local width = 160
  local height = #cheatsheet

  local opts = {
    style = 'minimal',
    relative = 'editor',
    width = width,
    height = height,
    row = (vim.o.lines - height) / 2,
    col = (vim.o.columns - width) / 2,
    border = 'shadow',
  }

  vim.api.nvim_open_win(buf, true, opts)
end

vim.api.nvim_create_user_command('Hlp', Hlp, {})
